...
int main()
{
    ...
    */
    ...
    // 암호화 과정(개인키)
    RSA_private_encrypt(RSA_size(rsa_pri), digest_padded, cipher_text, rsa_pri, RSA_NO_PADDING);
    printf("\n해시값 -> 암호문 : \n");
    for (int i = 0; i < 256; i++)
    {
    ...
    // // 복호화 과정(공개키)
    // num = RSA_public_decrypt(num, cipher_text, plain_text_receiver, key_pair, RSA_PKCS1_PADDING);
